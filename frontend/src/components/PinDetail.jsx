...
import {pinDetailQuery} from "../utills/data";
import {pinDetailMorePinQuery} from "../utills/data";
...
const PinDetail = ({ user }) => {
  ...
  return (
    ...
    <div className='flex xl:flex-row flex-col m-auto bg-white' style={{maxWidth: '1500px', borderRadius: '32px'}}>
      ...
      <div className='w-full p-5 flex-1 xl:min-w-620'>
        <div className='flex items-center justify-between '>
          ...
          <a href={ pinDetails.destination }  target="_blank" rel='noreferrer'>
            {pinDetails.destination?.slice(8,25)}
          </a>
        </div>
        <div>
      ...
      <Link to={`/userprofile/${pinDetails.postedBy?._id}`} className="flex gap-2 items-center mt-5 bg-white rounded-lg">
        <img
      ...
      <div className='flex flex-wrap mt-6 gap-3'>
        <Link to={`userprofile/${pinDetails.postedBy?._id}`} >
          <img
              className="w-10 h-10 rounded-full cursor-pointer"
              src={pinDetails.postedBy?.image}
              alt="user-profile"
